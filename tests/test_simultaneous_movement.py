...
# --- a flat movement cost


def test_a_move_costs_one_energy_from_any_starting_energy():
    for energy in (1, 2, 50, 99, 100):
        board = board_with([(1, 'u', 0, 0, 1, 5, energy)])
        order(board, ('u', UnitType.EAST))
        assert board.getUnitByName('u')[0].energy == energy - 1
...
def test_following_a_unit_that_moves_away_starts_no_contest():
    board = board_with([(1, 'lead', 1, 0, 4, 5, 4),
                        (2, 'chase', 2, 0, 3, 3, 1)])
...
def test_an_undecided_collision_leaves_both_where_they_started():
    board = facing(5, 5, 5, 5, energy=3)
    events = collide(board)
    a = board.getUnitByName('a')[0]
    b = board.getUnitByName('b')[0]
    # both paid the fare, neither could pay to attack
    assert a.energy == 2 and b.energy == 2
