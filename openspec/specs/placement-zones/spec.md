# placement-zones Specification

## Purpose

Where a player may deploy during setup. A two-player game gives each player
half the board to form up in, split by rows, with the middle row neutral where
the row count is odd; every other player count is the whole board. The area is
published per seat so a client can show it, and enforced when a unit is placed.
