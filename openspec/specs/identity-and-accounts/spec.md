...
Every session opens a game as a number, and `player-numbering` says what each
number may see and do. This says who may be that number. An account is one
person or one program that plays; a membership is which seat of which game
that account holds. Neither is something the rules of the game know about -
the engine resolves turns for player numbers and has never heard of an
account.

The two are kept apart deliberately. Because they are, the numbers and
everything the rules say about them are unchanged by the arrival of accounts:
what changed is only that a number reached through a server has to be proved
