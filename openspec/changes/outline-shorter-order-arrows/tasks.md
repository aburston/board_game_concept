...
- [ ] 1.1 In `src/board_game_concept/http/static/board.js`, rewrite
      ...
      by reading the function that it appends exactly one child and that the
      points for `north` are the `east` points with x and y swapped
- [ ] 1.2 Update the comment above `orderArrow` and the one above `REACH` so
...
- [ ] 2.1 Change `REACH` in `board.js` from `0.78` to `0.70` and verify
...
- [ ] 3.1 In `src/board_game_concept/http/static/style.css`, replace the
...
- [ ] 4.1 Add `tests/test_order_arrow.py`, reusing the `_static` and
...
- [ ] 4.2 In the same file, add a test that reads the `.board .order` rule
...
- [ ] 5.1 Run `pytest tests/test_order_arrow.py tests/test_board_selection.py
      tests/test_static_serving.py tests/test_web_flow.py` and then the full
      `pytest`, and verify both are green
- [ ] 5.2 Start the web interface, order a unit each way on a board where
