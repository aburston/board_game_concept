...
- [ ] 1.1 Replace `UnitType.MOVE_COST` with a read-only `move_cost` property on
...
- [ ] 1.2 Change `UnitType.planMove` to refuse on `self.energy < self.move_cost`
...
- [ ] 1.3 Verify with a new test that a unit of health 4 loses 4 energy to one
  ...
  energy.
- [ ] 1.4 Verify with a new test that a unit damaged to 1 health still pays its
  type's health for a move — the fare does not fall as the unit is worn down.

## 2. Type validation

- [ ] 2.1 Add the `energy >= health` assert to `UnitType.__init__`, guarded by
...
- [ ] 2.2 Verify with a test that a wall (attack 0, health 7, energy 0) is still
  created, and that attack 0 with energy 5 still fails with the wall message
  rather than the new one.
- [ ] 2.3 Verify at the CLI that `add type Heavy H 3 6 5` is refused through the
...
- [ ] 3.1 Wrap the `UnitType` construction in `Game._load_players` so an illegal
...
- [ ] 3.2 Apply the `energy = max(energy, health)` floor to the fallback branch
...
- [ ] 3.3 Verify that a record that does carry `type_*` fields is unaffected by
  3.2 — the floor applies only to the fallback.

## 4. Fixtures the new rule makes illegal

- [ ] 4.1 Redesign the twelve type definitions that now break the rule so each
...
- [ ] 4.2 Re-check `tests/player_1.yaml`, `tests/player_2.yaml`,
  ...
  comes back empty.
- [ ] 4.3 Re-run the whole suite (`pytest`) and the standalone harness
...
- [ ] 5.1 Rewrite `GAME_RULES.md` R4.3 so a move costs the unit's maximum
...
- [ ] 5.2 Add the new type rule (a non-wall type's energy is at least its
  ...
  consistently.
- [ ] 5.3 Update `SPEC_COVERAGE.md` if this change closes or opens any
  divergence it records; verify by re-reading its movement and unit-type
  entries against the new specs.

## 6. Sign-off

- [ ] 6.1 Verify the delta specs and the code agree: walk every scenario in
...
- [ ] 6.2 Run `openspec validate --strict` on the change and confirm it passes.
