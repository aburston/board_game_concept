...
- [ ] 1.1 Add a repository recorder that logs the operations one resolution
...
- [ ] 2.1 Split `resolve`'s per-player loop in two — what this turn published,
...
- [ ] 2.2 Verify `clear_commits` still precedes the loaded player's
      ...
      left to 1.1.
- [ ] 2.3 Verify resolution itself is unchanged: `tests/test_determinism.py`,
...
- [ ] 3.1 Add a test that a player released from waiting reads the turn they
...
- [ ] 3.2 Add a test that a session loading a game part way through a resolution
      ...
      of its own.
- [ ] 3.3 Verify a game set up with `load player` still gets its units onto the
...
- [ ] 4.1 Record the divergence in `SPEC_COVERAGE.md`, beside number 10 —
...
- [ ] 4.2 Run the full suite, `flake8 . --select=E9,F63,F7,F82` as CI does, and
...
- [ ] 4.3 Run the full suite ten times over and verify it is green every time.
