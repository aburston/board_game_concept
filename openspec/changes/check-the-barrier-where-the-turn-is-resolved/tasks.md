...
- [x] 1.1 Take the barrier condition out of `wait_for_all_commits` into
...
- [x] 2.1 Add `resolve_when_ready(game)`: hold the game for writing, read it,
...
- [x] 2.2 Give it three answers, not two: `None` for a barrier that is not met,
      ...
      a failure.
- [x] 2.3 Add the session operation beside `serverSave`, and verify
...
- [x] 3.1 Verify the case the change exists for: two callers each find the
      ...
      deterministically.
- [x] 3.2 Verify the turn resolved once — the turn number advanced by one, and
      each player's orders consumed once rather than twice.

## 4. The server asks instead of acting

- [x] 4.1 Have the unattended half of `cli/bgcserver.py` call it and handle all
...
- [x] 4.2 Verify the server still plays a game end to end and still reports an
      outcome:  `tests/test_server_client_integration.py`,
      `tests/test_cli_server_surface.py` and `tests/test_cli_outcome_surface.py`
      pass unedited.
- [x] 4.3 Verify a server woken for a turn somebody else resolved waits again
      rather than exiting, and says nothing about it.

## 5. Finish

- [x] 5.1 Record in `SPEC_COVERAGE.md` that the gap the previous entry left open
      is closed, naming the tests. Say what is still not one act: a player's
      commit does not resolve the turn.
- [x] 5.2 Run the full suite, `flake8 . --select=E9,F63,F7,F82` as CI does, and
...
- [x] 5.3 Run the full suite ten times over and verify it is green every time.
