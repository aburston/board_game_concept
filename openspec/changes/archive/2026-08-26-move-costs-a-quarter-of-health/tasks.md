...
- [x] 1.1 Change `UnitType.move_cost` to return `(self.type_health + 3) // 4`,
...
- [x] 1.2 Verify that `UnitType.planMove` and `Board._move` are untouched — both
...
- [x] 1.3 Verify with a test that a unit of health 4 loses 1 energy to a move, a
  unit of health 5 loses 2, a unit of health 10 loses 3, and a unit of health 1
  loses 1.
- [x] 1.4 Verify with a test that a unit damaged to 1 health still pays its
  type's fare — the fare does not fall as the unit is worn down.
- [x] 1.5 Verify with a test that no permitted health produces a fare of 0, so
  that no unit moves for nothing.

## 2. Type validation

- [x] 2.1 Move the `type_attack` / `type_health` / `type_energy` assignment
...
- [x] 2.2 Change the floor assert from `energy >= health` to
...
- [x] 2.3 Verify with a test that a type the old floor refused is now accepted —
  health 10 with energy 3 — and that no type legal under the old floor is
  refused by the new one.
- [x] 2.4 Verify with a test that a wall (attack 0, health 7, energy 0) is still
  created, and that attack 0 with energy 5 still fails with the wall message
  rather than the floor message.
- [x] 2.5 Verify at the CLI that `add type Heavy H 3 6 1` is refused through the
...
- [x] 3.1 Lower the reconstruction floor in `Game._typeFor` from
...
- [x] 3.2 Redesign the `test_a_stored_type_that_cannot_afford_a_move_is_refused`
...
- [x] 3.3 Verify that a record carrying `type_*` fields is still unaffected by
  the reconstruction floor.

## 4. The tests that assert the old fare

- [x] 4.1 Rewrite `tests/test_movement_cost.py` for the new fare: its module
...
- [x] 4.2 Re-scan `tests/` for any other assertion that a move costs the health
...
- [x] 4.3 Verify `tests/test_determinism.py` still passes, since head-on
  collisions now charge two amounts that may be equal where they could not be
  before.

## 5. Rules and the cost table

- [x] 5.1 Rewrite `GAME_RULES.md` R4.3 so a move costs a quarter of the unit's
...
- [x] 5.2 Rewrite the R9 cost table's movement rows and its "health is paid for
...
- [x] 5.3 Update `tests/test_cost_table.py` to hold R9 to the new fare, keeping
...
- [x] 5.4 Update `SPEC_COVERAGE.md` if this change closes or opens any
  divergence it records.

## 6. The match harness

- [x] 6.1 Move `matches/bots/common.py::fares` to the new fare so every doctrine
...
- [x] 6.2 Re-check `matches/bots/base.py::orders` and every doctrine's `floor`
...
- [x] 7.1 Verify the delta specs and the code agree: walk every scenario in
...
- [x] 7.2 Run the whole suite on both backends and the standalone harness
  (`python -m board_game_concept.test_suite`); verify all pass.
- [x] 7.3 Replay the twenty-game series against the new fare and write the
