...
- [x] 1.1 Route every write in `YamlGameRepository` through a helper that writes
...
- [x] 1.2 Verify what a write leaves behind is invisible to the three places
...
- [x] 1.3 Verify a write that does not finish leaves the previous contents
      readable — drive the helper to fail part way and confirm the target still
      holds what it held and the game still opens.

## 2. Holding a game

- [x] 2.1 Add `held(read=False)` to `GameRepository` as the operation a caller
...
- [x] 2.2 Make it re-entrant within a repository and bounded rather than
...
- [x] 2.3 Verify the sharing rules from two processes: two writers do not
...
- [x] 2.4 Verify a hold is released when its caller finishes, including when it
      finishes because of an error.
- [x] 2.5 Verify that where the platform offers no lock the repository carries
      on unheld and says so, rather than claiming a game was held
      (design.md — Decision 7).

## 3. Holding it where it matters

- [x] 3.1 Have `turn.resolve` and `turn.publish` hold the game for writing
...
- [x] 3.2 Have the shared reads in `Game.load` hold the game for reading, and
      ...
      against itself.
- [x] 3.3 Verify neither `wait_for_all_commits` nor `wait_for_turn` holds the
...
- [x] 3.4 Verify a turn still resolves, a game is still played end to end, and
...
- [x] 4.1 Update `MODULE_DESCRIPTION.md`'s account of storage to say a game can
      be held and that a write replaces rather than truncates. Verify every path
      it names exists.
- [x] 4.2 Record in `SPEC_COVERAGE.md` that the race divergence 10 tolerated,
...
- [x] 4.4 Run the full suite ten times over and verify it is green every time.
