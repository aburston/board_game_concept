...
- [x] 1.1 Add `segno` to `dependencies` in `pyproject.toml`, with a comment
      ...
      succeeds
- [x] 1.2 Extend `tests/test_packaging.py` with a test that imports `segno`
...
- [x] 2.1 In `src/board_game_concept/http/bgcapiserver.py`, add
      ...
      `127.0.0.1`
- [x] 2.2 Add `WILDCARDS = ('0.0.0.0', '::', '')` beside it and a helper
...
- [x] 3.1 Rewrite `_announce` to take its addresses from
      ...
      not
- [x] 3.2 Add `_qr(url)` that prints `segno.make(url).terminal(compact=True)`
      ...
      that none does
- [x] 3.3 Update the docstring of `_announce` and the module docstring so
...
- [x] 4.1 Add `tests/test_api_server_banner.py` with a fixture that builds
...
- [x] 4.2 In the same file, monkeypatch `reachable_address` to return
...
- [x] 4.3 In the same file, monkeypatch `reachable_address` to return `None`
...
- [x] 4.4 Run `pytest tests/test_api_server_banner.py tests/test_packaging.py
...
- [x] 5.1 Add a `## Playing in the same room` section to `README.md` under
...
- [x] 5.2 Add a `### Hosting from an Android phone` subsection under it:
      ...
      --host 0.0.0.0`; then what keeps it alive - `termux-wake-lock` (in
      Termux's base `termux-tools` package, or the "Acquire wakelock" action
      in the Termux notification), and turning off
...
- [x] 5.3 Reword the TLS entry under "Web service - what's next" to say
...
- [x] 6.1 On a laptop on a home Wi-Fi, run `bgcapiserver --host 0.0.0.0`,
...
- [x] 6.2 Verify the code scans from both a light and a dark terminal, and
      ...
      comment
- [x] 6.3 Run `bgcapiserver` with no flags and verify that nothing about the
...
Verification note: 6.1 and 6.2 were carried out without a phone to hand.
The server was bound wide and driven from the machine's network address
(not loopback) with two guest accounts through register, sign in, take a
seat, set up and one resolved turn; the code it drew was rebuilt from the
terminal output and decoded by a QR reader in both the dark-terminal and
light-terminal senses to exactly the printed URL. A camera reading a
physical screen is the one step not exercised.
