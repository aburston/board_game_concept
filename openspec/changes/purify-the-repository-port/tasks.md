...
- [ ] 1.1 Add `units_document(board, player=None, in_play_only=False, turn=None)`
...
- [ ] 1.2 Move the hand-crafted YAML emitter — the `serialise_units` and
...
- [ ] 1.3 Change the port signatures on `GameRepository` (the ABC in
      ...
      from 1.2 internally.
- [ ] 1.4 Update `service/turn.py`'s four writing callsites to build documents
...
- [ ] 1.5 Update every test that handed text to a write method or called the
...
- [ ] 2.1 Add `Notifier` (an ABC over `wake(name)` and `waiter(name)`),
...
- [ ] 2.2 Have `Game.__init__` take an optional `notifier`. When one is not
      ...
      `NullNotifier`.
- [ ] 2.3 Change `service/turn.py`'s four wake/waiter callsites to use
...
- [ ] 2.4 Remove `wake` and `waiter` from `GameRepository` (the ABC in
...
- [ ] 3.1 Verify `serialise_units`, `serialise_orders` and `_as_orders` are no
...
- [ ] 3.2 Update `MODULE_DESCRIPTION.md`'s account of `storage/`: the port
      ...
      names exists.
- [ ] 3.3 Run the full suite, `flake8 . --select=E9,F63,F7,F82` as CI does,
      ...
      report it before.
- [ ] 3.4 Run the full suite ten times over and verify it is green every time.
